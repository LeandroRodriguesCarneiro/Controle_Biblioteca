...
//    }
    ...
    public List<Loan> selectBooksByFilter(Integer idStudent) {
        MySQLConnector sql = new MySQLConnector();
        String query = "SELECT \r\n"
        		+ "	loa.id,\r\n"
        		...
        		+ "WHERE loa.status = 'Emprestimo' AND loa.id_student =" +idStudent;
        ...
        if (resultSet != null) {
            try {
                while (resultSet.next()) {
                    loanList.add(new Loan(resultSet.getInt("id"),resultSet.getInt("student_id"),resultSet.getDate("loan_date_init"),resultSet.getDate("loan_date_end"),
